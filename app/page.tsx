...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    register({
      ...
      inputSchema: { type: 'object', properties: { tab: { type: 'string', enum: ['Pulse', 'Galaxy', 'Chat', 'Profile'] } }, required: ['tab'], additionalProperties: false },
    ...
    register({
      name: 'like_current_profile', title: 'Like current profile', description: 'Like the profile currently visible in Pulse and open the match state.',
...
function FullProfile({ profile, open, onOpenChange, onPass, onLike, onShare }: { profile: Profile; open: boolean; onOpenChange: (open: boolean) => void; onPass: () => void; onLike: () => void; onShare: () => void }) {
...
function RoomsHub({ onOpenRoom }: { onOpenRoom: (name: string) => void }) {
  return <section className="screen scroll-screen"><header className="page-header"><p className="eyebrow">Find your frequency</p><h1>Galaxy</h1><p>Explore people in your orbit.</p></header><div className="room-grid">{roomData.map((item) => <button key={item.name} className={`room-tile ${item.className ?? ''}`} onClick={() => onOpenRoom(item.name)}><Image src={item.image} alt="" fill sizes="390px" className="profile-photo" /><span className="room-shade" /><span className="room-copy"><strong>{item.name}</strong><small>{item.caption}</small><em>{item.count}</em></span></button>)}</div><p className="stand-note"><Radio size={16} fill="currentColor" /> You can appear in Pulse + one Galaxy</p></section>;
}

function RoomStack({ room, profile, onBack, onOpen, onPass, onLike }: { room: string; profile: Profile; onBack: () => void; onOpen: () => void; onPass: () => void; onLike: () => void }) {
  return <section className="room-stack"><header className="room-header"><button onClick={onBack} aria-label="Back to Galaxy"><ArrowLeft size={22} /></button><div><strong>{room}</strong><span>{room === 'Tonight' ? '84 here now' : 'Live now'}</span></div><span className="live-dot" /></header><div className="context-chip">{room === 'Tonight' ? 'Free after 8' : `Into ${room.toLowerCase()}`}</div><div className="room-card-wrap"><ProfileCard profile={profile} room={room} onOpen={onOpen} onSwipeLeft={onPass} onSwipeRight={onLike} /></div><ActionRow onPass={onPass} onLike={onLike} /></section>;
}
...
function ProfilePreview({ name, onBack }: { name: string; onBack: () => void }) {
  ...
  return <section className="preview-screen"><header className="preview-banner"><button onClick={onBack} aria-label="Back to profile"><ArrowLeft size={21} /></button><span><strong>This is how you appear</strong><small>What people see in Pulse</small></span></header><div className="preview-wrap"><ProfileCard profile={self} preview /><button className="edit-card"><Edit3 size={18} /> Edit card</button></div></section>;
}
